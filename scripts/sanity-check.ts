 ...
 *
 * Boots two nodes and verifies:
 * 1. Services start correctly
 * 2. Peer discovery works
 * 3. ECIES handshake completes
 * 4. Status exchange occurs
 * 5. ETH protocol messages work
 * 6. Sync completes and RPC starts
 * 7. Transactions are processed
 * 8. Chain consistency between nodes
 *
 * The databases (including Ethash cache) are pre-generated by generate-snapshot.ts.
 * Each run copies the snapshot databases for a clean, consistent state.
 *
 * Usage:
...
async function checkChainConsistency(): Promise<{
  ...
  try {
    ...
    if (node1Latest && node2Latest) {
      ...
      return { passed: node1Height === node2Height && hashesMatch, details }
    }
