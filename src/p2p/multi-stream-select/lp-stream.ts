...
export class LengthPrefixedStream {
  ...
  constructor (stream: AbstractMessageStream, options: LengthPrefixedStreamOptions = {}) {
    this.stream = stream
   ...
   */
  async write (data: Uint8Array | Uint8ArrayList, options?: AbortOptions): Promise<void> {
    options?.signal?.throwIfAborted()

    const length = data instanceof Uint8ArrayList ? data.byteLength : data.length
   ...
   */
  async writeV (messages: Array<Uint8Array | Uint8ArrayList>, options?: AbortOptions): Promise<void> {
    options?.signal?.throwIfAborted()

    const output = new Uint8ArrayList()
    for (const msg of messages) {
      const length = msg instanceof Uint8ArrayList ? msg.byteLength : msg.length
    ...
    try {
      ...
      while (offset < this.buffer.byteLength && offset < this.maxLengthLength) {
        ...
        if (offset >= this.maxLengthLength) {
          throw new Error(`Varint too long (max ${this.maxLengthLength} bytes)`)
        }
      }
      ...
      if (length > this.maxDataLength) {
        throw new Error(`Message length ${length} exceeds max ${this.maxDataLength}`)
      }
  ...
  private async waitForData (options?: AbortOptions): Promise<Uint8Array | Uint8ArrayList> {
...
export function lpStream (stream: AbstractMessageStream, options?: LengthPrefixedStreamOptions): LengthPrefixedStream {
