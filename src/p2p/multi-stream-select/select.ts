 ...
 * ```
 */
export async function select <Stream extends AbstractMessageStream> (stream: Stream, protocols: string | string[], options: MultistreamSelectInit = {}): Promise<string> {
  ...
  throw new Error(`Protocol selection failed - could not negotiate ${protocols}`)
}
