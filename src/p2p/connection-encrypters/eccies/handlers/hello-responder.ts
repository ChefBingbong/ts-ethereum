...
const log = debug("p2p:ecies:hello-responder");
...
export function waitHelloSendHello(
	ctx: HelloContext,
	timeoutMs = 10000,
): Promise<HelloResult> {
	return new Promise((resolve, reject) => {
				...
				while (socketData.length >= nextPacketSize) {
					if (state === "header") {
						...
						if (code === PREFIXES.HELLO) {
							...
							log(
								"Received HELLO (responder): clientId=%s",
								remoteHello.clientId,
							);
							...
							if (!sent) {
								reject(new Error("Failed to send HELLO response"));
								return;
							}

							const localHello = parseHelloPayload(helloPayload);
							log(
								"Sent HELLO response (responder): clientId=%s",
								localHello.clientId,
							);
		...
		const onTimeout = () => {
			cleanup();
			reject(new Error(`HELLO timeout after ${timeoutMs}ms (responder)`));
		};
