...
export interface TCPSocketMultiaddrConnectionInit
	extends Omit<
		AbstractMultiaddrConnectionInit,
		"name" | "stream" | "remoteAddr"
	> {
	socket: Socket;
		...
		if (init.localAddr != null && Unix.matches(init.localAddr)) {
			...
			if (init.socket.remoteAddress == null || init.socket.remotePort == null) {
				...
				throw new InvalidParametersError(
					"Could not determine remote address or port",
				);
			}
...
export const toMultiaddrConnection = (
	init: TCPSocketMultiaddrConnectionInit,
): MultiaddrConnection => {
