...
const isDirect = (multiaddr: Multiaddr): boolean => {
	return (
		multiaddr
			.getComponents()
			.find((component) => component.code === CODE_P2P) != null
	);
};
...
export class Connection extends BasicConnection {
	...
	override async newStream(
		...
		if (this.muxer.status !== "open") {
			throw new Error(
				`The connection muxer is "${this.muxer.status}" and not "open"`,
			);
		}
		...
		this.log(
			"started new stream %s for protocols %s",
			muxedStream.id,
			protocols,
		);
		...
		try {
			if (options.signal == null) {
				...
				debugLog(
					"connection.ts:103",
					"Created AbortSignal.timeout for outbound protocol negotiation",
					{
						timeout: this.outboundStreamProtocolNegotiationTimeout,
						protocols: protocols.join(","),
						streamId: muxedStream.id,
					},
					"B",
				);
				signal.addEventListener("abort", () => {
					debugLog(
						"connection.ts:103",
						"AbortSignal fired for outbound protocol negotiation",
						{
							protocols: protocols.join(","),
							streamId: muxedStream.id,
			...
			if (muxedStream.protocol === "") {
				muxedStream.log.trace(
					"selecting protocol from protocols %s",
					protocols,
				);

				muxedStream.protocol = await mss.select(
					muxedStream,
					protocols,
					options,
				);
			...
			const outgoingLimit = this.findOutgoingStreamLimit(
				muxedStream.protocol,
				options,
			);
			...
			if (streamCount > outgoingLimit) {
				const err = new Error(
					`Too many outbound protocol streams for protocol "${muxedStream.protocol}" - ${streamCount}/${outgoingLimit}`,
				);
			...
			if (muxedStream.status === "open") {
				...
				this.log.error(
					"could not create new outbound stream on connection %s %s for protocols %s - %s",
					this.direction === "inbound" ? "from" : "to",
					this.remoteAddr.toString(),
					protocols,
					err.message,
				);
			}
	...
	private async onIncomingStream(evt: CustomEvent<any>): Promise<void> {
		...
		debugLog(
			"connection.ts:146",
			"Created AbortSignal.timeout for inbound protocol negotiation",
			{
		...
		signal.addEventListener("abort", () => {
			debugLog(
				"connection.ts:146",
				"AbortSignal fired for inbound protocol negotiation",
				{
					streamId: muxedStream.id,
					reason: signal.reason,
				},
		...
		muxedStream.log(
			"start protocol negotiation, timing out after %dms",
			this.inboundStreamProtocolNegotiationTimeout,
		);
		...
		try {
			...
			if (streamCount > incomingLimit) {
				throw new Error(
					`Too many inbound protocol streams for protocol "${muxedStream.protocol}" - limit ${incomingLimit}`,
				);
			}

			const { handler } = this.components.registrar.getHandler(
				muxedStream.protocol,
			);
	...
	private findOutgoingStreamLimit(
		protocol: string,
		options: NewStreamOptions = {},
	): number {
		try {
