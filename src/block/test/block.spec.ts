...
import { createLegacyTx, paramsTx } from "../../tx";
...
describe("[Block]: block functions", () => {
	...
	it("should test transaction validation - transaction not signed", async () => {
		...
		const maxTransactionGasLimit = paramsTx["7825"]
			.maxTransactionGasLimit as number;
	...
	it("should test transaction validation - invalid tx trie", async () => {
		...
		try {
			...
			assert.isTrue(
				(error.message as string).includes("invalid transaction trie"),
			);
		}
	});
	...
	it("should test hash() method (mainnet default)", () => {
		...
		assert.deepEqual(
			block.hash(),
			new Uint8Array([1]),
			"custom crypto applied on hash() method",
		);
	});
	...
	it("should return the same block data from raw()", () => {
		...
		const block = createBlockFromRLP(
			hexToBytes(preLondonTestDataBlocks2RLP.block2RLP),
			{
				common,
			},
		);
		const createBlockFromRaw = createBlockFromBytesArray(block.raw(), {
			common,
		});
	...
	it("DAO hardfork", () => {
		const blockData = RLP.decode(
			preLondonTestDataBlocks2RLP.block0RLP,
		) as NestedUint8Array;
	...
	it("should set canonical difficulty if I provide a calcDifficultyFromHeader header", () => {
		...
		assert.strictEqual(
			blockWithDifficultyCalculation.header.ethashCanonicalDifficulty(
				genesis.header,
			),
			blockWithDifficultyCalculation.header.difficulty,
	...
	it("should be able to initialize shanghai blocks with correct hardfork defaults", () => {
		...
		assert.deepEqual(
			block.withdrawals,
			[],
			"withdrawals should be set to default empty array",
		);
	});
});
