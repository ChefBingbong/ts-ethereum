import type { NumericString, PrefixedHexString } from "../utils/index.ts";
import { bigIntToHex } from "../utils/index.ts";
import type { ExecutionPayload } from "./index.ts";
 ...
 */
export function executionPayloadFromBeaconPayload(
	...
	if (
		payload.excess_blob_gas !== undefined &&
		payload.excess_blob_gas !== null
	) {
		executionPayload.excessBlobGas = bigIntToHex(
			BigInt(payload.excess_blob_gas),
		);
	}
	if (
		payload.parent_beacon_block_root !== undefined &&
		payload.parent_beacon_block_root !== null
	) {
