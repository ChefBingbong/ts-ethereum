...
export class Heap {
	...
	constructor(opts?: QHeapOptions | Function) {
		...
		this._freeSpace =
			opts.freeSpace === undefined ? this._trimArraySize : false;
	...
	public gc(options?: { minLength?: number; minFull?: number }): void {
		...
		if (
			this._list.length >= minListLength &&
			this.length < this._list.length * minListFull
		) {
	...
	public _check(): boolean {
		...
		for (i = this.length; i > 1; i--) {
			...
			if (
				_compar(this._list[p], this._list[i]) > 0 &&
