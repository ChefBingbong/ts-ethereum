...
> extends Readable {
	...
	private success(
		...
		if (reenqueue) {
			...
			if (job.result !== undefined) {
				this.DEBUG &&
					this.debug(
						`Successful job completion job ${jobStr}, writing to out and dequeue`,
					);
				this.out.insert(job);
				this.dequeue();
			} else {
	...
	private failure(
		job:
			| Job<JobTask, JobResult, StorageItem>
			| Job<JobTask, JobResult, StorageItem>[],
		error?: Error,
	...
	next() {
		...
		if (this.processed === this.total) {
			this.DEBUG &&
				this.debug(
					`Total number of tasks reached, skip job ${jobStr} execution.`,
				);
			return false;
		}
	...
	write() {
			...
			try {
				...
				if (!destroyFetcher) {
					...
					if (this.isBlockFetcherJobTask(jobItems[0].task)) {
						this.DEBUG &&
							this.debug(
								`Possible reorg, stepping back ${stepBack} blocks and requeuing jobs.`,
							);
		...
		const writer = new Writable({
			...
			writev: (
				many: {
					chunk: Job<JobTask, JobResult, StorageItem>;
					encoding: string;
				}[],
				cb: Function,
			) => {
	...
	expire(job: Job<JobTask, JobResult, StorageItem>) {
		...
		if (this.pool.contains(job.peer!)) {
			...
			this.DEBUG &&
				this.debug(
					`Peer disconnected while performing task ${jobStr} ${job.peer}`,
				);
		}
		this.enqueue(job);
	}
	...
	private isBlockFetcherJobTask(
		task: JobTask | BlockFetcherJobTask,
	): task is BlockFetcherJobTask {
		if (task === undefined || task === null) return false;
		const keys = Object.keys(task);
		return (
			keys.filter((key) => key === "first" || key === "count").length === 2
		);
	}
}
