...
import { errorCodes, fastify, FastifyInstance, FastifyRequest } from "fastify";
 ...
 */
const INVALID_MEDIA_TYPE_CODE =
	errorCodes.FST_ERR_CTP_INVALID_MEDIA_TYPE().code;
...
export class RestApiServer {
	...
	constructor(
		...
		if (opts.bearerToken) {
			void server.register(bearerAuthPlugin, {
				keys: new Set([opts.bearerToken]),
			});
		}
		...
		server.addHook("onError", async (req, _res, err) => {
			...
			if (
				...
				[INVALID_MEDIA_TYPE_CODE, SCHEMA_VALIDATION_ERROR_CODE].includes(
					err.code,
				)
			) {
				this.logger.warn(`Req ${req.id} ${operationId} failed`, {
					reason: err.message,
				});
			} else {
	...
	async listen(): Promise<void> {
		try {
			...
			this.logger.info("Started REST API server", {
