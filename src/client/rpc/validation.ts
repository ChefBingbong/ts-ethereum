...
function bytes(bytes: number, params: any[], index: number) {
	...
	if (
		...
		!/^[0-9a-fA-F]+$/.test(params[index].substr(2))
	) {
		return {
			code: INVALID_PARAMS,
			message: `invalid argument ${index}: argument must be a hex string`,
...
function uint(uint: number, params: any[], index: number) {
	if (uint % 8 !== 0) {
		...
		throw EthereumJSErrorWithoutCode(
			`Uint should be a multiple of 8, got: ${uint}`,
		);
	}
	if (typeof params[index] !== "string") {
		return {
...
export const validators = {
	...
	get blockOption() {
		return (params: any[], index: number) => {
			...
			if (
				!["latest", "finalized", "safe", "earliest", "pending"].includes(
					blockOption,
				)
			) {
	...
	get withdrawalRequest() {
		return (
			requiredFields: string[] = ["sourceAddress", "validatorPubkey", "amount"],
		) => {
			return (params: any[], index: number) => {
				if (typeof params[index] !== "object") {
					return {
