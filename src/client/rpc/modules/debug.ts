...
import { type VM, encodeReceipt, runTx } from "../../../vm";
...
export class Debug {
	...
	constructor(client: EthereumClient, rpcDebug: boolean) {
		...
		this.traceCall = middleware(
			callWithStackTrace(this.traceCall.bind(this), this._rpcDebug),
			2,
		...
		this.setHead = middleware(
			callWithStackTrace(this.setHead.bind(this), this._rpcDebug),
			1,
			[[validators.blockOption]],
		);
		this.verbosity = middleware(
			callWithStackTrace(this.verbosity.bind(this), this._rpcDebug),
			1,
			[[validators.unsignedInteger]],
		);
	}
	...
	async traceTransaction(params: [PrefixedHexString, tracerOpts]) {
		...
		const txHashIndex = await this.service.execution.txIndex.getIndex(
			hexToBytes(txHash),
		);
		if (!txHashIndex) return null;
		const result =
			await this.service.execution.receiptsManager.getReceiptByTxHashIndex(
				txHashIndex,
			);
		if (!result) return null;
		const [_, blockHash, txIndex] = result;
		const block = await this.service.chain.getBlock(blockHash);
		const parentBlock = await this.service.chain.getBlock(
		...
		vmCopy.evm.events?.on("step", async (step, next) => {
			...
			const log = {
				...
				error: null,
				stack:
					opts.disableStack !== true ? step.stack.map(bigIntToHex) : undefined,
				storage,
				memory,
