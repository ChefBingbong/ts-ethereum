...
function logFormat(colors = false) {
	return printf(
		(info: { level: string; message: unknown; [key: string]: unknown }) => {
			...
			if (colors) {
				const color =
					chalk[LevelColors[info.level as keyof typeof LevelColors]];
				level = color(level);
			...
			if (info.attentionCL !== undefined)
				attentionCL = info.attentionCL as string;
			if (info.attentionHF !== undefined)
				attentionHF = info.attentionHF as string;
