...
export type EthProtocolVersion = typeof ETH_PROTOCOL_VERSIONS[keyof typeof ETH_PROTOCOL_VERSIONS];
...
export const ETH_MESSAGE_CODE_NAMES: Record<EthMessageCode, string> = {
  ...
  [EthMessageCode.NEW_POOLED_TRANSACTION_HASHES]: "NEW_POOLED_TRANSACTION_HASHES",
...
export const ETH_MESSAGES = {
  [EthMessageCode.STATUS]: {
    ...
    encode: (args: {
      ...
      return {
        ...
        latestBlock: args.latestBlock !== undefined ? bigIntToUnpaddedBytes(args.latestBlock) : undefined,
      };
    },
    decode: (data: { chainId: Uint8Array; td: Uint8Array; bestHash: Uint8Array; genesisHash: Uint8Array; latestBlock?: Uint8Array }) => {
      return {
        chainId: bytesToBigInt(data.chainId),
        td: bytesToBigInt(data.td),
        bestHash: data.bestHash,
        genesisHash: data.genesisHash,
        latestBlock: data.latestBlock ? bytesToBigInt(data.latestBlock) : undefined,
      };
    },
  },
  ...
  [EthMessageCode.TRANSACTIONS]: {
    ...
    decode: (txs: Uint8Array[], config: { synchronized: boolean; chainCommon: any }) => {
  ...
  [EthMessageCode.NEW_BLOCK]: {
    ...
    decode: ([block, td]: [BlockBytes, Uint8Array], config: { chainCommon: any }) => [
  ...
  [EthMessageCode.BLOCK_HEADERS]: {
    ...
    decode: ([reqId, headers]: [Uint8Array, BlockHeaderBytes[]], config: { chainCommon: any }) => [
      bytesToBigInt(reqId),
      headers.map((h) => {
  ...
  [EthMessageCode.GET_BLOCK_BODIES]: {
    ...
    encode: ({ reqId, hashes }: { reqId?: bigint; hashes: Uint8Array[] }, nextReqId: { value: bigint }) => [
      bigIntToUnpaddedBytes(reqId ?? ++nextReqId.value),
      hashes,
    ],
  ...
  [EthMessageCode.NEW_POOLED_TRANSACTION_HASHES]: {
    ...
    encode: (
      ...
      return isNestedUint8Array(params) === true
        ? params
        : [
            bytesToHex(new Uint8Array(params[0])),
            params[1],
            params[2],
          ];
    },
  ...
  [EthMessageCode.POOLED_TRANSACTIONS]: {
    ...
    decode: ([reqId, txs]: [Uint8Array, any[]], config: { chainCommon: any }) => {
      const common = config.chainCommon.copy();
      return [
        bytesToBigInt(reqId),
        txs.map((txData) => {
...
export const ETH_VERSION_CAPABILITIES: Record<EthProtocolVersion, EthVersionCapability> = {
  ...
  [ETH_PROTOCOL_VERSIONS.ETH67]: {
    ...
    supportedMessages: ETH65_MESSAGES.filter(
      code => code !== EthMessageCode.GET_NODE_DATA && code !== EthMessageCode.NODE_DATA
    ),
...
export const ETH_TRANSPORT_REQUIREMENTS: Record<TransportType, TransportRequirement> = {
  [TransportType.RLPX]: {
...
export function isMessageSupported(
  ...
  if (message.maxVersion !== undefined && version > message.maxVersion) return false;
  
  return true;
}

export function getSupportedMessages(version: EthProtocolVersion): EthMessageCode[] {
  return ETH_VERSION_CAPABILITIES[version].supportedMessages;
}

export function getEthMessage<TCode extends EthMessageCode>(
  code: TCode
) {
  return ETH_MESSAGES[code] 
}

export function getMessageDefinition(code: EthMessageCode): EthMessageDefinition {
  return ETH_MESSAGES[code];
}

export function getResponseCode(requestCode: EthMessageCode): EthMessageCode | undefined {
...
export function getVersionCapability(version: EthProtocolVersion): EthVersionCapability {
