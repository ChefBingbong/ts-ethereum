import { EthereumJSErrorWithoutCode, Lock } from "../../../utils";
...
export class BoundProtocol {
			...
			try {
				if (this.peer.pooled) {
					...
					if (this.messageQueue.length >= 50) {
						const error = EthereumJSErrorWithoutCode(
							"unexpected message queue growth for peer",
						);
	...
	private handle(incoming: Message) {
		...
		try {
			...
			error = EthereumJSErrorWithoutCode(
				`Could not decode message ${message.name}: ${e}`,
			);
		}
...
export class BoundEthProtocol
	extends BoundProtocol
	implements EthProtocolMethods
{
	name = "eth";
