	...
	private async initRlpx() {
		return new Promise<void>((resolve) => {
			...
			this.rlpx.events.on("peer:added", async (rlpxPeer: Devp2pRLPxPeer) => {
				const isInbound = (rlpxPeer["_socket"] as any).server !== undefined;
				const remoteAddress = rlpxPeer["_socket"].remoteAddress!;
				const remotePort = rlpxPeer["_socket"].remotePort!;
				const peerId = rlpxPeer.getId()!;

				let peer: RlpxPeer | null = new RlpxPeer({
					config: this.config,
					id: bytesToUnprefixedHex(peerId),
					host: remoteAddress,
					port: remotePort,
				...
				try {
					...
					if (isInbound && this.dpt) {
						...
						const helloMsg = rlpxPeer.getHelloMessage();
			...
			this.rlpx.events.on(
				"peer:removed",
				(rlpxPeer: Devp2pRLPxPeer, reason: any) => {
			...
			this.rlpx.events.on(
				"peer:error",
				(rlpxPeer: Devp2pRLPxPeer, error: Error) => this.error(error),
			);
			...
			if (typeof this.config.port === "number") {
				this.rlpx.listen(this.config.port, "127.0.0.1");
			}
		});
	}
}
