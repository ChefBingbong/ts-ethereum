...
import { createWalletClient, defineChain, formatEther, http, parseEther, publicActions, type Hex } from 'viem'
...
function loadAccounts(): AccountInfo[] {
  const accountsPath = path.resolve(__dirname, '../../../test-network-data/accounts.json')
  
  if (!existsSync(accountsPath)) {
    console.error(`Accounts file not found: ${accountsPath}`)
    console.error('Make sure to run the test network first to generate accounts.')
    process.exit(1)
  }
...
async function main() {
  ...
  if (Number.isNaN(toIndex)) {
    ...
    console.error('')
    console.error('Usage: npx tsx src/client/bin/send-funds.ts --from 0 --to 1 --amount 1.5')
    console.error('')
  ...
  if (fromIndex < 0 || fromIndex >= accounts.length) {
    console.error(`Error: --from index ${fromIndex} is out of range (0-${accounts.length - 1})`)
    process.exit(1)
  }
  
  if (toIndex < 0 || toIndex >= accounts.length) {
    console.error(`Error: --to index ${toIndex} is out of range (0-${accounts.length - 1})`)
    process.exit(1)
  }
  ...
  console.log(`    Change: ${formatEther(fromBalanceAfter - fromBalanceBefore)} ETH`)
  console.log(`  ${toAccount.role} (${toAccount.address}):`)
  console.log(`    ${formatEther(toBalanceAfter)} ETH`)
  console.log(`    Change: +${formatEther(toBalanceAfter - toBalanceBefore)} ETH`)
