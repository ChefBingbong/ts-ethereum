...
const SHARED_DIR = "../../test-network-data-p2p";
...
function createGenesisState(accounts: Account[]): Record<string, string> {
...
async function startClient() {
	...
	console.log("\n" + "=".repeat(60));
	console.log(`🚀 Starting Ethereum Test Network Node (New P2P Stack)`);
	console.log(`   Port: ${port}`);
	console.log(`   Is Bootnode: ${isBootnode}`);
	console.log(`   Clean Start: ${cleanStart}`);
	console.log(`   P2P Stack: NEW (Transport + Mplex + Multi-stream-select)`);
	console.log("=".repeat(60) + "\n");
	...
	const config = new Config({
		...
		useP2PServer: true, // ✨ Enable new P2P stack!
	});
	...
	config.events.on(Event.PEER_CONNECTED, (peer) => {
		...
		console.log(`   Using new P2P stack ✨\n`);
	});
	...
	console.log(`   P2P Stack: NEW (Transport + Mplex + MSS) ✨`);
	console.log("=".repeat(60) + "\n");
