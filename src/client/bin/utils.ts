...
import { parseMultiaddrs } from "../util/index.ts";
			...
			.check((argv, _options) => {
				...
				if (collision)
					throw EthereumJSErrorWithoutCode(
						"cannot reuse ports between RPC instances",
					);
				return true;
			})
			.parseSync()
...
export async function getCryptoFunctions(
	useJsCrypto: boolean,
): Promise<CustomCrypto> {
...
export async function generateClientConfig(args: ClientOpts) {
	...
	const chainName =
		args.chainId ?? args.networkId ?? args.network ?? Chain.Mainnet;
	...
	if (typeof args.customChain === "string") {
		try {
			const customChainParams = JSON.parse(
				readFileSync(args.customChain, "utf-8"),
			);
			...
			throw EthereumJSErrorWithoutCode(
				`invalid chain parameters: ${err.message}`,
			);
		}
	...
	const multiaddrs =
		args.multiaddrs !== undefined
			? parseMultiaddrs(args.multiaddrs)
			: undefined;
	...
	console.log("bootnodes", bootnodes, multiaddrs);
