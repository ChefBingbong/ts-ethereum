...
import { ROOT_DB_KEY, type BinaryTreeOpts } from './types.ts'
...
/**
 * The basic binary tree interface, use with `import { BinaryTree } from '../../binarytree'`.
 *
...
export class BinaryTree {
  ...
  constructor(opts: BinaryTreeOpts) {
    ...
    this.DEBUG =
      typeof globalThis === 'undefined' ? (process?.env?.DEBUG?.includes('ethjs') ?? false) : false
  ...
  async findPath(keyInBytes: Uint8Array): Promise<Path> {
    ...
    while (!finished) {
      ...
      if (rawNode === undefined)
        throw new Error(`missing node at ${childNode.path}`)
