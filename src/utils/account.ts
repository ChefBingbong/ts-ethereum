 ...
 */
function handleNullIndicator(
	values: NestedUint8Array | Uint8Array,
): Uint8Array | null {
	...
	if (nullIndicator > 1) {
		throw new Error(`Invalid isNullIndicator=${nullIndicator}`);
	}
	if (values.length < 2) {
...
export class Account {
	...
	get version() {
		if (this._version !== null) {
			...
			throw new Error(`version=${this._version} not loaded`);
		}
	}
	...
	get nonce() {
		if (this._nonce !== null) {
			...
			throw new Error(`nonce=${this._nonce} not loaded`);
		}
	}
	...
	get balance() {
		if (this._balance !== null) {
			...
			throw new Error(`balance=${this._balance} not loaded`);
		}
	}
	...
	get storageRoot() {
		if (this._storageRoot !== null) {
			...
			throw new Error(`storageRoot=${this._storageRoot} not loaded`);
		}
	}
	...
	get codeHash() {
		if (this._codeHash !== null) {
			...
			throw new Error(`codeHash=${this._codeHash} not loaded`);
		}
	}
	...
	get codeSize() {
		if (this._codeSize !== null) {
			...
			throw new Error(`codeSize=${this._codeSize} not loaded`);
		}
	}
	...
	isContract(): boolean {
		if (this._codeHash === null && this._codeSize === null) {
			throw new Error(`Insufficient data as codeHash=null and codeSize=null`);
		}
		return (
			(this._codeHash !== null &&
...
export function createAccount(accountData: AccountData) {
	...
	if (
		...
		throw new Error(`Partial fields not supported in fromAccountData`);
	}
...
export function createPartialAccount(partialAccountData: PartialAccountData) {
	const { nonce, balance, storageRoot, codeHash, codeSize, version } =
		partialAccountData;
	...
	if (
		...
		throw new Error(`All partial fields null`);
	}
...
export function createPartialAccountFromRLP(serialized: Uint8Array) {
	...
	for (const value of values) {
		...
		if (!Array.isArray(value)) {
			throw new Error("Invalid partial encoding. Each item must be an array");
		}
	}
...
export const generateAddress2 = function (
	...
	const address = keccak256(
		concatBytes(hexToBytes("0xff"), from, salt, keccak256(initCode)),
	);
	...
	if (publicKey.length === 64) {
		...
		try {
			secp256k1.ProjectivePoint.fromHex(
				concatBytes(Uint8Array.from([4]), publicKey),
			);
			return true;
		} catch {
			return false;
