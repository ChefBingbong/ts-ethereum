...
export function decodeBinaryAccessState(
	...
	const position =
		BigInt(treeIndex) * BigInt(BINARY_TREE_NODE_WIDTH) + BigInt(chunkIndex);
	switch (position) {
		case BigInt(0):
		...
		default:
			if (position < BINARY_TREE_HEADER_STORAGE_OFFSET) {
				throw Error(
					`No attribute yet stored >=2 and <${BINARY_TREE_HEADER_STORAGE_OFFSET}`,
				);
			}

			if (
				position >= BINARY_TREE_HEADER_STORAGE_OFFSET &&
...
export class BinaryTreeAccessWitness
	implements BinaryTreeAccessWitnessInterface
{
	...
	constructor(opts: {
		...
		this.stems =
			opts.stems ??
			new Map<PrefixedHexString, BinaryStemAccessEvent & BinaryStemMeta>();
		this.chunks =
			opts.chunks ?? new Map<PrefixedHexString, BinaryChunkAccessEvent>();
	}

	readAccountBasicData(address: Address): bigint {
		return this.touchAddressOnReadAndComputeGas(
			address,
			0,
			BINARY_TREE_BASIC_DATA_LEAF_KEY,
		);
	...
	writeAccountBasicData(address: Address): bigint {
		return this.touchAddressOnWriteAndComputeGas(
			address,
			0,
			BINARY_TREE_BASIC_DATA_LEAF_KEY,
		);
	...
	readAccountCodeChunks(
		...
		let gas = BIGINT_0;
		for (
			let chunkNum = Math.floor(startPc / 31);
			chunkNum <= Math.floor(endPc / 31);
			chunkNum++
		) {
			const { treeIndex, subIndex } =
				getBinaryTreeIndicesForCodeChunk(chunkNum);
			gas += this.touchAddressOnReadAndComputeGas(
				contract,
				treeIndex,
				subIndex,
			);
	...
	writeAccountCodeChunks(
		...
		for (
			...
			const { treeIndex, subIndex } =
				getBinaryTreeIndicesForCodeChunk(chunkNum);
			gas += this.touchAddressOnWriteAndComputeGas(
				contract,
				treeIndex,
				subIndex,
			);
	...
	readAccountStorage(address: Address, storageSlot: bigint): bigint {
		const { treeIndex, subIndex } =
			getBinaryTreeIndicesForStorageSlot(storageSlot);
	...
	touchAddressAndComputeGas(
		...
		const { stemRead, stemWrite, chunkRead, chunkWrite, chunkFill } =
			this.touchAddress(address, treeIndex, subIndex, { isWrite });
	...
	touchAddress(
		...
		const accessedStemKey = getBinaryTreeStem(
			this.hashFunction,
			address,
			treeIndex,
		);
		...
		let accessedStem =
			this.stemCache.get(accessedStemHex) ?? this.stems.get(accessedStemHex);
		...
		let accessedChunk =
			this.chunkCache.get(accessedChunkKeyHex) ??
			this.chunks.get(accessedChunkKeyHex);
	...
	merge(accessWitness: BinaryTreeAccessWitness): void {
		for (const [chunkKey, chunkValue] of accessWitness.chunks.entries()) {
			...
			if (stem === undefined) {
				throw Error(
					`Internal error: missing stem for the chunkKey=${chunkKey}`,
				);
			}
