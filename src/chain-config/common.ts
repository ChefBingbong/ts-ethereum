...
export class Common {
	...
	constructor(opts: CommonOpts) {
		...
		this.DEFAULT_HARDFORK =
			this._chainParams.defaultHardfork ?? Hardfork.Prague;
	...
	setHardfork(hardfork: string | Hardfork): void {
		...
		if (!existing) {
			throw Error(`Hardfork with name ${hardfork} not supported`);
		}
	}
	...
	getHardforkBy(opts: HardforkByOpts): string {
		...
		if (timestamp !== undefined) {
			...
			if (maxTimeStamp < timestamp) {
				throw Error(
					`Maximum HF determined by block number is lower than timestamp HF`,
				);
			}
		}
		...
		if (this.gteHardfork(Hardfork.Bpo1)) {
			return {
				targetBlobGasPerBlock:
					this.param("target") * this.param("blobGasPerBlob"),
	...
	consensusType(): string | ConsensusType {
		...
		let value: string | ConsensusType | undefined;
		for (const hfChanges of this.HARDFORK_CHANGES) {
