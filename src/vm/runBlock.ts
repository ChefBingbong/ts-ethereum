...
export async function runBlock(vm: VM, opts: RunBlockOpts): Promise<RunBlockResult> {
  ...
  if (vm.DEBUG) {
    debug('-'.repeat(100))
    debug(`Running block hash=${bytesToHex(block.hash())} number=${block.header.number}`)
  }
...
async function applyTransactions(vm: VM, block: Block, opts: RunBlockOpts) {
  ...
  const receiptTrie =
    block.transactions.length > 0 ? new MerklePatriciaTrie({ common: vm.common }) : undefined
