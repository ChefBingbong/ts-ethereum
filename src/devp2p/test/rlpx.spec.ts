...
describe("RLPx", () => {
	...
	it("should connect to a peer", async () => {
		vi.mock("net", () => {
			// Provide a constructible mock Socket for `new net.Socket()`
			class MockSocket {
				on = vi.fn();
				once = vi.fn();
				setTimeout = vi.fn();
		...
		vi.mock("../util.ts", async () => {
			const util: any = await vi.importActual("../util.ts");
			return {
				...util,
		...
		const mockOnConnect = vi
			.spyOn(rlpx, "_onConnect")
			.mockImplementation((_, peerId) => {
				assert(
					equalsBytes(peerId as Uint8Array, mockPeer.id),
					"received correct peerId",
				);
			});
