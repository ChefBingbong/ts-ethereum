...
export class DPT {
	...
	constructor(privateKey: Uint8Array, options: DPTOptions) {
		...
		const refreshIntervalSubdivided = Math.floor(
			(options.refreshInterval ?? 60000) / 10,
		); // 60 sec * 1000
	...
	/**
	 * Add a peer that has already been verified (e.g., via RLPx connection).
	 * This skips the UDP ping check since we know the peer is alive.
	 * Useful for adding inbound peers to enable peer discovery propagation.
	 */
	addVerifiedPeer(obj: PeerInfo): boolean {
		if (this._banlist.has(obj)) {
			if (this.DEBUG) {
				this._debug(
					`addVerifiedPeer: peer is banned ${obj.address}:${obj.udpPort}`,
				);
			}
			return false;
		}

		// check k-bucket first - don't add duplicates
		const existing = this._kbucket.get(obj);
		if (existing !== null) {
			if (this.DEBUG) {
				this._debug(
					`addVerifiedPeer: peer already exists ${obj.address}:${obj.udpPort}`,
				);
			}
			return false;
		}
		...
		if (this.DEBUG) {
			this._debug(
				`addVerifiedPeer: adding verified peer ${obj.address}:${obj.udpPort}`,
			);
		}

		// Add directly to kbucket without ping
		this._kbucket.add(obj);
		this.events.emit("peer:new", obj);

		// Mark as confirmed since it's a verified RLPx connection
		if (obj.id !== undefined) {
			this._confirmedPeers.add(bytesToUnprefixedHex(obj.id));
		}
	...
	getClosestPeers(id: Uint8Array) {
		...
		if (this._onlyConfirmed && this._confirmedPeers.size > 0) {
			peers = peers.filter((peer) =>
				this._confirmedPeers.has(bytesToUnprefixedHex(peer.id as Uint8Array))
					? true
					: false,
			);
		}
