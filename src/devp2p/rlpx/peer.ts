...
export const PrefixesNames: { [key in PREFIXES]: string } = Object.entries(
	PREFIXES,
).reduce(
	(acc, [key, value]) => {
...
export class Peer {
	...
	_sendPing() {
		if (this.DEBUG) {
			this.debug(
				"PING",
				`Send PING to ${this._socket.remoteAddress}:${this._socket.remotePort}`,
			);
		}
		let data = RLP.encode([]);
	...
	_sendPong() {
		if (this.DEBUG) {
			this.debug(
				"PONG",
				`Send PONG to ${this._socket.remoteAddress}:${this._socket.remotePort}`,
			);
		}
		let data = RLP.encode([]);
	...
	_handleHeader() {
		...
		this.DEBUG &&
			this._logger(
				`Received header ${this._socket.remoteAddress}:${this._socket.remotePort}`,
			);
	...
	_addFirstPeerDebugger() {
		...
		if (typeof ip === "string") {
			this._logger = devp2pDebug
				.extend(ip)
				.extend(`FIRST_PEER`)
				.extend(DEBUG_BASE_NAME);
		}
	}
