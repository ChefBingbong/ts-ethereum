...
export class RLPx {
	...
	_onConnect(socket: net.Socket, peerId: Uint8Array | null) {
		if (this.DEBUG) {
			this._debug(
				`connected to ${socket.remoteAddress}:${socket.remotePort}, handshake waiting..`,
			);
		}
		...
		if (peerId === null && this._getOpenSlots() === 0) {
			peer.events.once("connect", () =>
				peer.disconnect(DISCONNECT_REASON.TOO_MANY_PEERS),
			);
			socket.once("error", () => {});
			return;
		}
		...
		peer.events.once("close", (reason, disconnectWe) => {
			...
			if (
				disconnectWe !== true &&
				reason === DISCONNECT_REASON.TOO_MANY_PEERS
			) {
				// hack
	...
	_refillConnections() {
		...
		this._refillIntervalSelectionCounter =
			(this._refillIntervalSelectionCounter + 1) % 10;
