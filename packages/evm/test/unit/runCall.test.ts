...
import { defaultBlock } from '../../src/evm'
...
*/
  ...
  it('Constantinople: EIP-1014 CREATE2 creates the right contract address', async () => {
    ...
    */
    ...
    await evm.stateManager.putAccount(
      caller,
      new Account(BigInt(0), BigInt(0x11111111)),
    ) // give the calling account a big balance so we don't run out of funds
    ...
    for (let value = 0; value <= 1000; value += 20) {
      ...
      const expectedAddress = create2address(
        contractAddress,
        codeHash,
        valueBytes,
      )
      ...
      const executionReturnValue = new Address(
        res.execResult.returnValue.slice(12),
      )
    ...
    assert.isTrue(
      true,
      'CREATE2 creates (empty) contracts at the expected address',
    )
  })
  ...
  it('Byzantium cannot access Constantinople opcodes', async () => {
    ...
    */
    ...
    await evmConstantinople.stateManager.putCode(
      contractAddress,
      hexToBytes(code),
    ) // setup the contract code
  ...
  */
  ...
  it('runCall() -> skipBalance behavior', async () => {
    ...
    const contractAddress = createAddressFromString(
      '0x000000000000000000000000636F6E7472616374',
    )
    await evm.stateManager.putCode(contractAddress, contractCode)

    // TODO: SIGNER_G from @ethereumjs/testdata is not available
    // Using a test address instead
    const sender = createAddressFromString(
    ...
    for (const balance of [undefined, BigInt(5)]) {
      await evm.stateManager.modifyAccountFields(sender, {
        nonce: BigInt(0),
        balance,
      })
      const res = await evm.runCall(runCallArgs)
      assert.isTrue(
        true,
        'runCall should not throw with no balance and skipBalance',
      )
  ...
  it.skip('runCall() => use BLOBHASH opcode from EIP 4844', async () => {
    // TODO: eip4844GethGenesis from @ethereumjs/testdata is not available
    // setup the evm
  ...
  it.skip('runCall() => use BLOBBASEFEE opcode from EIP 7516', async () => {
    // TODO: eip4844GethGenesis from @ethereumjs/testdata is not available
    // setup the evm
    // const common = createCommonFromGethGenesis(eip4844GethGenesis, {
    //   chain: 'custom',
  ...
  it('ensure call and callcode handle gas stipend correctly', async () => {
    ...
    for (const [opcode, gas, expectedOutput] of [
       ...
       * CALLCODE/CALL
       */
      const callCodeAddress = createAddressFromString(
      ...
      const callerAddress = createAddressFromString(
        '0x000000000000000000000000000000000000aaab',
      )
      ...
      await evm.stateManager.putAccount(
        callerAddress,
        new Account(undefined, BigInt(1)),
      )
      ...
      assert.strictEqual(
        callResult,
        expectedOutput,
        `should have result ${expectedOutput}`,
      )
    }
  })
})
