...
export class Interpreter {
  ...
  async run(
    ...
    if (pc !== 0 && (pc < 0 || pc >= this._runState.code.length)) {
      throw EthereumJSErrorWithoutCode(
        'Internal error: program counter not in range',
      )
    }
    ...
    if (
      this.profilerOpts?.enabled === true &&
      this.performanceLogger.hasTimer()
    ) {
    ...
    while (this._runState.programCounter < this._runState.code.length) {
      ...
      if (doJumpAnalysis) {
        ...
        if (opCode === 0x56 || opCode === 0x57 || opCode === 0x5e) {
          const { jumps, pushes, opcodesCached } =
            this._getValidJumpDestinations(this._runState.code)
  ...
  async storageStore(key: Uint8Array, value: Uint8Array): Promise<void> {
    ...
    if (!account) {
      throw EthereumJSErrorWithoutCode(
        'could not read account while persisting memory',
      )
    }
