...
function getAdjustedExponentLength(
  ...
  firstExpBigInt =
    firstExpBigInt >> (BIGINT_8 * BigInt(Math.max(max32expLen, 0)))
...
export function precompile05(opts: PrecompileInput): ExecResult {
  ...
  const B = bytesToBigInt(
    setLengthRight(data.subarray(Number(bStart), Number(bEnd)), Number(bLen)),
  )
  const E = bytesToBigInt(
    setLengthRight(data.subarray(Number(eStart), Number(eEnd)), Number(eLen)),
  )
  const M = bytesToBigInt(
    setLengthRight(data.subarray(Number(mStart), Number(mEnd)), Number(mLen)),
  )
