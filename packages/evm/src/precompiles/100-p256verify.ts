...
import { bytesToBigInt, bytesToHex, setLengthLeft } from '@ts-ethereum/utils'
...
const SUCCESS_RETURN = new Uint8Array(32)
  .fill(0)
  .map((_, i) => (i === 31 ? 1 : 0))
...
export function precompile100(opts: PrecompileInput): ExecResult {
  ...
  if (data.length !== 160) {
    if (opts._debug !== undefined) {
      opts._debug(
        `${pName} failed: invalid input length ${data.length}, expected 160`,
      )
    }
    return {
  ...
  const rightSide =
    (qxBigInt * qxBigInt * qxBigInt + P256_A * qxBigInt + P256_B) % P256_P
