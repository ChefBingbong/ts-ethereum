...
import type { EVMBLSInterface } from '../../types'
import {
  BLS_FIELD_MODULUS,
...
function BLS12_381_ToFp2Point(
  fpXCoordinate: Uint8Array,
  fpYCoordinate: Uint8Array,
) {
  ...
  const fpBytes = concatBytes(
    fpXCoordinate.subarray(16),
    fpYCoordinate.subarray(16),
  )
...
function BLS12_381_ToG1Point(input: Uint8Array, verifyOrder = true) {
  ...
  try {
    ...
    if (
      verifyOrder ||
      (e as Error).message !== 'bad point: not in prime-order subgroup'
    )
...
export class NobleBLS implements EVMBLSInterface {
  ...
  mapFP2toG2(input: Uint8Array): Uint8Array {
    ...
    const Fp2Point = BLS12_381_ToFp2Point(
      input.subarray(0, 64),
      input.subarray(64, 128),
    )
    const result = bls12_381.G2.mapToCurve([Fp2Point.c0, Fp2Point.c1])
  ...
  msmG1(input: Uint8Array): Uint8Array {
    ...
    for (let k = 0; k < numPairs; k++) {
      ...
      const Fr = BLS12_381_ToFrPoint(
        input.subarray(
          pairStart + BLS_G1_POINT_BYTE_LENGTH,
  ...
  msmG2(input: Uint8Array): Uint8Array {
    ...
    for (let k = 0; k < numPairs; k++) {
      ...
      const Fr = BLS12_381_ToFrPoint(
        input.subarray(
          pairStart + BLS_G2_POINT_BYTE_LENGTH,
          pairStart + pairLength,
        ),
      )
      let pMul
  ...
  pairingCheck(input: Uint8Array): Uint8Array {
    ...
    const filteredPairs = pairs.filter(
      (pair) =>
        pair.g1.equals(G1_ZERO) === false && pair.g2.equals(G2_ZERO) === false,
    )
