...
import { FORMAT } from './eof/constants'
import { isEOF } from './eof/util'
import { EVMError } from './errors'
import type { InterpreterOpts } from './interpreter'
import { Interpreter } from './interpreter'
import { Journal } from './journal'
import type { Timer } from './logger'
...
export class EVM implements EVMInterface {
  ...
  constructor(opts: EVMOpts) {
    ...
    if (this.common.isEIPActiveAtHardfork(2537, this.fork)) {
  ...
  protected async _executeCall(message: MessageWithTo): Promise<EVMResult> {
    ...
    if (this.common.isEIPActiveAtHardfork(7864, this.fork)) {
      ...
      if (message.depth === 0) {
        ...
        debugGas(
          `originAccessGas=${originAccessGas} waived off for origin at depth=0`,
        )

        let destAccessGas = message.accessWitness.readAccountCodeHash(
          message.to,
        )
        if (sendsValue) {
          destAccessGas += message.accessWitness.writeAccountBasicData(
            message.to,
          )
        } else {
          destAccessGas += message.accessWitness.readAccountBasicData(
            message.to,
          )
        }

        debugGas(
          `destAccessGas=${destAccessGas} waived off for target at depth=0`,
        )
      }
      ...
      if (gasLimit < BIGINT_0) {
        if (this.DEBUG) {
          debugGas(
            `callAccessGas charged(${callAccessGas}) caused OOG (-> ${gasLimit})`,
          )
        }
    ...
    if (message.isCompiled) {
      ...
      if (this._optsCached.profiler?.enabled === true) {
        this.performanceLogger.stopTimer(
          timer!,
          Number(result.executionGasUsed),
          'precompiles',
        )
  ...
  protected async _executeCreate(message: Message): Promise<EVMResult> {
      ...
      if (
        ...
        return {
          ...
          execResult: {
            returnValue: new Uint8Array(0),
            exceptionError: new EVMError(
              EVMError.errorMessages.INITCODE_SIZE_VIOLATION,
            ),
    ...
    let result = await this.runInterpreter({
      ...message,
      gasLimit,
      isCreate: true,
    } as Message)
    ...
    if (
      ...
      returnFee =
        ...
        BigInt(this.common.getParamAtHardfork('createDataGas', this.fork)!)
    ...
    if (
      ...
      result.returnValue.length >
        Number(this.common.getParamAtHardfork('maxCodeSize', this.fork)!)
    ) {
    ...
    if (
      ...
      if (
        this.common.isEIPActiveAtHardfork(3541, this.fork) &&
        result.returnValue[0] === FORMAT
      ) {
        if (!this.common.isEIPActiveAtHardfork(3540, this.fork)) {
  ...
  protected async runInterpreter(
    ...
    let contract = await this.stateManager.getAccount(
      message.to ?? createZeroAddress(),
    )
    if (!contract) {
    ...
    const interpreterRes = await interpreter.run(
      message.code as Uint8Array,
      opts,
    )
    ...
    if (interpreterRes.exceptionError) {
      if (
        ...
        interpreterRes.exceptionError.error !==
