...
export class StatefulBinaryTreeStateManager implements StateManagerInterface {
  ...
  getAccount = async (address: Address): Promise<Account | undefined> => {
    ...
    if (accountValues[0] !== null && accountValues[0] !== undefined) {
      ...
      if (this.DEBUG) {
        this._debug(
          `getAccount address=${address.toString()} from DB (non-existent)`,
        )
      }
    ...
    if (this.DEBUG) {
      this._debug(
        `getAccount address=${address.toString()} stem=${short(stem)}`,
      )
    }
    return account
  }
  ...
  public initBinaryTreeExecutionWitness(
    ...
    const postStateRaw = executionWitness.stateDiff.flatMap(
      ({ stem, suffixDiffs }) => {
        const suffixDiffPairs = suffixDiffs.map(
          ({ newValue, currentValue, suffix }) => {
            const key =
              `${stem}${padToEven(Number(suffix).toString(16))}` as PrefixedHexString
    ...
    this._debug(
      `initBinaryTreeExecutionWitness postState=${JSON.stringify(this._postState)}`,
    )
  }
  ...
  putCode = async (address: Address, value: Uint8Array): Promise<void> => {
    ...
    const chunkStems = generateBinaryTreeCodeStems(
      codeChunks.length,
      address,
      this.hashFunction,
    )

    const chunkSuffixes: number[] = generateBinaryTreeChunkSuffixes(
      codeChunks.length,
    )
    ...
    await this._tree.put(
      ...
      codeChunks.slice(
        0,
        codeChunks.length <= BINARY_TREE_CODE_OFFSET
          ? codeChunks.length
          : BINARY_TREE_CODE_OFFSET,
      ),
    )
    ...
    for (let stem = 1; stem < chunkStems.length; stem++) {
      const sliceStart =
        BINARY_TREE_CODE_OFFSET + BINARY_TREE_NODE_WIDTH * (stem - 1)
      const sliceEnd =
  ...
  getCode = async (address: Address): Promise<Uint8Array> => {
    ...
    chunksByStem[0] = await this._tree.get(
      ...
      chunkSuffixes.slice(
        0,
        codeSize <= BINARY_TREE_CODE_OFFSET
          ? codeSize
          : BINARY_TREE_CODE_OFFSET,
      ),
    )
    ...
    for (let x = 0; x < chunks.length; x++) {
      if (chunks[x] === undefined)
        throw EthereumJSErrorWithoutCode(
          `expected code chunk at index ${x}, got undefined`,
        )
      ...
      if (x === chunks.length - 1) {
        ...
        lastChunkByteIndex =
          codeSize % BINARY_TREE_CODE_CHUNK_SIZE || BINARY_TREE_CODE_CHUNK_SIZE
      }
      code.set(
  ...
  getStorage = async (
    ...
    const value = await this._tree.get(storageKey.slice(0, 31), [
      storageKey[31],
    ])
    ...
    switch (type) {
      ...
      case BinaryTreeAccessedStateType.CodeHash: {
        if (this._caches === undefined) {
          ...
          return bytesToHex(
            createPartialAccountFromRLP(encodedAccount).codeHash,
          )
        }
      }
  ...
  async verifyBinaryTreePostState(
    ...
    for (const accessedState of accessWitness?.accesses() ?? []) {
      ...
      let canonicalValue: PrefixedHexString | null | undefined =
        this._postState[chunkKey]
      ...
      if (accessedState.type === BinaryTreeAccessedStateType.Code) {
        computedValue =
          computedValue !== null ? `0x${computedValue.slice(4)}` : null
        canonicalValue =
          canonicalValue !== null ? `0x${canonicalValue.slice(4)}` : null
      } else if (
    ...
    for (const canChunkKey of Object.keys(this._postState)) {
      if (accessedChunks.get(canChunkKey) === undefined) {
        this.DEBUG &&
          this._debug(`Missing chunk access for canChunkKey=${canChunkKey}`)
        postFailures++
      }
    }
  ...
  async checkChunkWitnessPresent(
    _address: Address,
    _codeOffset: number,
  ): Promise<boolean> {
    throw EthereumJSErrorWithoutCode('Method not implemented.')
