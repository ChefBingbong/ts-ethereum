...
export const dumpLeafValues = async (
  ...
  if (node instanceof StemBinaryNode) {
    ...
    const res = (await Promise.all(childPaths)).filter(
      (val) => val !== undefined,
    )
...
export const dumpNodeHashes = async (
  ...
  equalsBytes(startingNode, tree.root()) &&
    entries.push(['0x', bytesToHex(startingNode)])
  if (node instanceof InternalBinaryNode) {
    ...
    const childPaths = (
      await Promise.all(
        children.map((value) => dumpNodeHashes(tree, value.hash)),
      )
    )
