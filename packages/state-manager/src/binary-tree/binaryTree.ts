...
export class BinaryTree {
  ...
  root(value?: Uint8Array | null): Uint8Array {
    if (value !== undefined) {
      ...
      if (value.length !== this._hashLen) {
        throw EthereumJSErrorWithoutCode(
          `Invalid root length. Roots are ${this._hashLen} bytes`,
        )
      }
  ...
  async get(
    ...
    if (stem.length !== 31)
      throw EthereumJSErrorWithoutCode(
        `expected stem with length 31; got ${stem.length}`,
      )
    this.DEBUG &&
      this.debug(`Stem: ${bytesToHex(stem)}; Suffix: ${suffixes}`, ['get'])
    ...
    if (stemPath.node instanceof StemBinaryNode) {
      ...
      for (const suffix of suffixes) {
        ...
        this.DEBUG &&
          this.debug(
            `Suffix: ${suffix}; Value: ${value === null ? 'null' : bytesToHex(value)}`,
            ['get'],
          )
        values.push(value)
      }
  ...
  async put(
    ...
    if (
      ...
      this.DEBUG &&
        this.debug(`Creating new stem node for stem: ${bytesToHex(stem)}`, [
          'put',
        ])
    }
    ...
    if (stemNode.values.every((val) => val === null)) {
      if (foundPath.node !== null) {
        this.DEBUG &&
          this.debug(`Deleting stem node for stem: ${bytesToHex(stem)}`, [
            'put',
          ])
    ...
    if (foundPath.stack.length > 1) {
      ...
      const updated = this.updateBranch(
        stemNode,
        nearestNode,
        nearestNodePath,
        parentPath,
    ...
    while (foundPath.stack.length > 1) {
      ...
      if (isInternalBinaryNode(node)) {
        ...
        this.DEBUG &&
          this.debug(
            `Updated parent internal node hash for path ${path.join(',')}`,
            ['put'],
          )
      } else {
        throw EthereumJSErrorWithoutCode(
    ...
    if (isStemBinaryNode(rootNode)) {
      ...
      if (!equalsBytes(rootNode.stem, stem)) {
        this.DEBUG &&
          this.debug(`Root stem differs from new stem. Splitting root.`, [
            'put',
          ])
        ...
        for (let depth = commonPrefixLength - 1; depth >= 0; depth--) {
          this.DEBUG &&
            this.debug(`Creating internal node at depth ${depth}`, ['put'])
        ...
        rootNode.setChild(
          ...
          childReference !== null
            ? {
                ...
                path: isStemBinaryNode(childReference)
                  ? stemBits
                  : lastUpdatedParentPath,
              }
            : null,
        )
      }
    ...
    this.DEBUG &&
      this.debug(`Updated root hash to ${bytesToHex(this._root)}`, ['put'])
  ...
  updateBranch(
    ...
    if (isStemBinaryNode(nearestNode)) {
      ...
      const putStack = [
        { node: newInternal, parentPath: stemBits.slice(0, diffIndex) },
      ]
  ...
  async findPath(keyInBytes: Uint8Array): Promise<Path> {
    ...
    this.DEBUG &&
      this.debug(`Searching for key: ${bytesToHex(keyInBytes)}`, ['find_path'])
    const result: Path = {
      node: null,
      stack: [],
      remaining: keyInBits,
    ...
    this.DEBUG &&
      this.debug(`Starting with Root Node: [${bytesToHex(this.root())}]`, [
        'find_path',
      ])
  ...
  async del(stem: Uint8Array, suffixes: number[]): Promise<void> {
    this.DEBUG &&
      this.debug(`Stem: ${bytesToHex(stem)}; Suffix(es): ${suffixes}`, ['del'])
  ...
  async createRootNode(): Promise<void> {
    const rootNode = null
    this.DEBUG &&
      this.debug(`No root node. Creating new root node`, ['initialize'])
    this.root(this.merkelize(rootNode))
  ...
  async createBinaryProof(key: Uint8Array): Promise<Uint8Array[]> {
    this.DEBUG &&
      this.debug(`creating proof for ${bytesToHex(key)}`, ['create_proof'])
  ...
  protected hash(msg: Uint8Array | null): Uint8Array {
    ...
    if (
      msg === null ||
      (msg.length === 64 && msg.every((byte) => byte === 0))
    ) {
  ...
  protected merkelize(node: BinaryNode | null): Uint8Array {
    ...
    while (currentLayerHashes.length > 1) {
      ...
      for (let i = 0; i < currentLayerHashes.length; i += 2) {
        newLayerHashes.push(
          this.hash(
            concatBytes(currentLayerHashes[i], currentLayerHashes[i + 1]),
          ),
        )
      }
    ...
    return this.hash(
      concatBytes(setLengthRight(node.stem, 32), currentLayerHashes[0]),
    )
  }
}
