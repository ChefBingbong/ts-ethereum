...
export class MerklePatriciaTrie {
  ...
  constructor(opts?: MPTOpts) {
    ...
    if (opts !== undefined) {
      ...
      valueEncoding =
        opts.db !== undefined
          ? (opts.valueEncoding ?? ValueEncoding.String)
          : ValueEncoding.Bytes
    } else {
  ...
  async get(
    ...
    const { node, remaining } = await this.findPath(
      this.appliedKey(key),
      throwIfMissing,
    )
    ...
    this.DEBUG &&
      this.debug(`Value: ${value === null ? 'null' : bytesToHex(value)}`, [
        'get',
      ])
    return value
  }
  ...
  async put(
    ...
    this.DEBUG &&
      this.debug(`Value: ${value === null ? 'null' : bytesToHex(key)}`, ['put'])
    if (
  ...
  async findPath(
    ...
    for (let i = 0; i < partialPath.stack.length - 1; i++) {
      ...
      progress +=
        stack[i] instanceof BranchMPTNode
          ? 1
          : (stack[i] as ExtensionMPTNode).keyLength()
    }
    this.DEBUG &&
      this.debug(`Target (${targetKey.length}): [${targetKey}]`, ['find_path'])
      ...
      if (node instanceof BranchMPTNode) {
        ...
        for (const k of node.key()) {
          if (k !== targetKey[progress]) {
            result = {
              node: null,
              remaining: targetKey.slice(_progress),
              stack,
            }
            return
          }
        ...
        this.DEBUG &&
          this.debug(
            `Comparing node key to expected\n|| Node_Key: [${node.key()}]\n|| Expected: [${targetKey.slice(
              ...
              targetKey
                .slice(progress, progress + node.key().length)
                .toString() === node.key().toString()
            }]
            `,
        ...
        for (const k of node.key()) {
          this.DEBUG &&
            this.debug(`NextNode: ${node.value()}`, [
              'find_path',
              'extension_node',
            ])
          if (k !== targetKey[progress]) {
    ...
    const start =
      startingNode !== undefined
        ? this.hash(startingNode.serialize())
        : this.root()
    try {
      this.DEBUG &&
        this.debug(
    ...
    for await (const { node, currentKey } of this.walkTrieIterable(
      ...
      async (node) => {
        return (
          ...
          (node instanceof BranchMPTNode && node.value() !== null)
        )
      },
    )) {
      await onFound(node, currentKey)
  ...
  protected async _createInitialNode(
    ...
    rootKey = this._opts.keyPrefix
      ? concatBytes(this._opts.keyPrefix, rootKey)
      : rootKey
  ...
  async lookupNode(node: Uint8Array | Uint8Array[]): Promise<MPTNode> {
    if (isRawMPTNode(node)) {
      ...
      this.DEBUG &&
        this.debug(`${decoded.constructor.name}`, ['lookup_node', 'raw_node'])
      return decoded
    }
    this.DEBUG &&
      this.debug(`${`${bytesToHex(node)}`}`, ['lookup_node', 'by_hash'])
    const key = this._opts.keyPrefix
      ? concatBytes(this._opts.keyPrefix, node)
      : node
    ...
    this.DEBUG &&
      this.debug(`${decoded.constructor.name} found in DB`, [
        'lookup_node',
        'by_hash',
      ])
    return decoded
  ...
  protected async _updateNode(
    ...
    if (lastNode instanceof LeafMPTNode) {
      ...
      if (
        ...
        matchingNibbleLength(lastNode.key(), key.slice(l)) ===
          lastNode.key().length
      ) {
        matchLeaf = true
      }
  ...
  protected async _deleteNode(k: Uint8Array, stack: MPTNode[]): Promise<void> {
    const processBranchMPTNode = (
      ...
      if (
        ...
        parentNode instanceof BranchMPTNode
      ) {
        // branch->?
        ...
        if (branchNode instanceof BranchMPTNode) {
          ...
          const extensionNode = new ExtensionMPTNode(
            [branchKey],
            new Uint8Array(),
          )
    ...
    const branchNodes: [number, NodeReferenceOrRawMPTNode][] =
      lastNode.getChildren()
    ...
    if (branchNodes.length === 1) {
      ...
      if (this._opts.useNodePruning) {
        ...
        opStack.push({
          type: 'del',
          key: isRawMPTNode(branchNode)
            ? this.appliedKey(RLP.encode(branchNode))
            : branchNode,
        })
      }
  ...
  _formatNode(
    ...
    if (encoded.length >= 32 || topLevel) {
      ...
      const key = this._opts.keyPrefix
        ? concatBytes(this._opts.keyPrefix, lastRoot)
        : lastRoot
  ...
  protected async _findDbNodes(onFound: FoundNodeFunction): Promise<void> {
    const outerOnFound: FoundNodeFunction = async (
      nodeRef,
      node,
      key,
      walkController,
  ...
  protected hash(msg: Uint8Array): Uint8Array {
    return Uint8Array.from(
      this._opts.useKeyHashingFunction.call(undefined, msg),
    )
  }
  ...
  flushCheckpoints() {
    this.DEBUG &&
      this.debug(`Deleting ${this._db.checkpoints.length} checkpoints.`, [
        'flush_checkpoints',
      ])
  ...
  async getValueMap(
    ...
    await this.walkAllValueNodes(
      async (node: MPTNode, currentKey: number[]) => {
        if (node instanceof LeafMPTNode) {
          const keyBytes = nibblesTypeToPackedBytes(
            currentKey.concat(node.key()),
          )
          if (!inRange) {
