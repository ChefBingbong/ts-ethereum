...
export abstract class AbstractStream extends AbstractMessageStream {
	...
	async close(options?: AbortOptions): Promise<void> {
		...
		if (this.writableNeedsDrain) {
			this.log(
				"waiting for write queue to drain before closing writable end of stream, %d unsent bytes, sending %s",
				this.writeBuffer.byteLength,
				this.sendingData,
			);
			...
			this.log(
				"write queue drained, closing writable end of stream, %d unsent bytes, sending %s",
				this.writeBuffer.byteLength,
				this.sendingData,
			);
		}
