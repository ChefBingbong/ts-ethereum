...
export class ECIES {
  ...
  constructor(
    ...
    this._ephemeralPublicKey = secp256k1.getPublicKey(
      this._ephemeralPrivateKey,
      false,
    )

    this._keccakFunction = keccak256
    this._ecdsaSign = secp256k1.sign
  ...
  _setupFrame(remoteData: Uint8Array, incoming: boolean): void {
    ...
    this._ingressMac.update(
      concatBytes(xor(macSecret, this._nonce), remoteData),
    )
    this._egressMac = new MAC(macSecret)

    if (this._initMsg === null || this._initMsg === undefined) return
    this._egressMac.update(
      concatBytes(xor(macSecret, this._remoteNonce), this._initMsg),
    )
  }
  ...
  parseAuthPlain(
    ...
    this._ephemeralSharedSecret = ecdhX(
