...
export class RLPxTransport implements Transport<RLPxDialEvents> {
  ...
  async dial(ma: Multiaddr, options: RLPxDialOptions) {
    ...
    try {
      ...
      // Wrap RLPxConnection with adapter to implement libp2p Connection interface
      return new RLPxConnectionAdapter(
        connection,
        this.components.logger.forComponent('rlpx:transport'),
      ) as any
    } catch (err: any) {
