...
export class RLPxConnection extends EventEmitter<RLPxConnectionEvents> {
  ...
  constructor(options: RLPxConnectionOptions) {
    ...
    this._remoteId = options?.remoteId ?? null
  ...
  private _handleHello(payload: any): void {
    ...
    // Allow connections with no protocols for testing/transport-level connections
    // In production, you should have at least one protocol capability
    if (this._protocols.length === 0) {
      this.log(
        'No protocols negotiated with %s:%d - allowing connection for transport testing',
        this._socket.remoteAddress,
        this._socket.remotePort,
      )
      // Don't disconnect - allow the connection to complete for transport testing
      // In production, you would typically disconnect here:
      // this.disconnect(DISCONNECT_REASON.USELESS_PEER)
      // return
    }
