...
/**
 * RLPx Listener Events - extends standard ListenerEvents
 * Note: Uses standard 'connection' event with adapted connection for libp2p compatibility
 */
export interface RLPxListenerEvents extends ListenerEvents {
   ...
   * The connection is wrapped with RLPxConnectionAdapter for libp2p compatibility
   */
  connection: CustomEvent<import('./connection-adapter').RLPxConnectionAdapter>
}
  ...
  implements Listener
{
  ...
  private onSocket(socket: net.Socket): void {
    ...
    connection.once('connect', () => {
      ...
      // Wrap connection with adapter for libp2p compatibility
      const adaptedConnection = new RLPxConnectionAdapter(
        connection,
        this.context.logger.forComponent('rlpx:listener'),
      )

      // Emit the connection event with adapted connection
      this.safeDispatchEvent('connection', {
        detail: adaptedConnection,
      })
