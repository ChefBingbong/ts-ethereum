...
export class Transport {
	...
	private readonly connectionCache: Map<string, BasicConnection | Connection> =
		new Map();
	private readonly inFlightDials = new Map<
		string,
		SafePromise<BasicConnection | Connection>
	>();
	...
	async dialBasic(
		...
		const dialPromise = this.scheduleDial(
			async (): SafePromise<BasicConnection | Connection> => {
				...
				return await new Promise<
					SafeError<Error> | SafeResult<BasicConnection>
				>((resolve) => {
					const cleanup = () => {
						clearTimeout(timer);
	...
	async dialFull(
		...
		remotePeerId?: Uint8Array,
		timeoutMs = 60_000,
	): SafePromise<Connection> {
		const [basicError, basicConn] = await this.dialBasic(
			peerAddr,
			remotePeerId,
			timeoutMs,
		);
	...
	private async scheduleDial(
		dialCallback: () => SafePromise<BasicConnection | Connection>,
	): SafePromise<BasicConnection | Connection> {
	...
	private async onConnect(
		...
		peerAddr: Multiaddr,
		remotePeerId?: Uint8Array,
	): Promise<SafeError<Error> | SafeResult<BasicConnection>> {
		try {
			log(
				`📡 TCP connected to ${peerAddr.toString()}, creating BasicConnection...`,
			);
			...
			log(
				`✅ BasicConnection created: ${basicConn.id}, remote peer: ${bytesToHex(basicConn.remotePeer).slice(0, 18)}...`,
			);
			...
			log(
				`❌ BasicConnection creation failed for ${peerAddr.toString()}: ${err.message}`,
			);
	...
	private checkExistingConnection(
		peerAddr: Multiaddr,
	): SafeResult<BasicConnection | Connection> | null {
		const cacheKey = peerAddr.toString();
	...
	createListener(
		context: Omit<ListenerContext, "upgrader">,
	): TransportListener {
