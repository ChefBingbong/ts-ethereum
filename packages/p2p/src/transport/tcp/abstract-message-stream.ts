	...
	implements MessageStream
{
	...
	constructor(init: MessageStreamInit) {
		...
		this.maxReadBufferLength =
			init.maxReadBufferLength ?? DEFAULT_MAX_READ_BUFFER_LENGTH;
	...
	async *[Symbol.asyncIterator](): AsyncGenerator<Uint8Array | Uint8ArrayList> {
		...
		const streamAsyncIterableOnMessageListener = (
			evt: StreamMessageEvent,
		): void => {
			output.push(evt.data);
		};
		this.addEventListener("message", streamAsyncIterableOnMessageListener);

		const streamAsyncIterableOnCloseListener = (
			evt: StreamCloseEvent,
		): void => {
		...
		this.addEventListener(
			"remoteCloseWrite",
			streamAsyncIterableOnRemoteCloseWriteListener,
		);
	...
	send(data: Uint8Array | Uint8ArrayList): boolean {
		if (this.writeStatus === "closed" || this.writeStatus === "closing") {
			throw new StreamStateError(
				`Cannot write to a stream that is ${this.writeStatus}`,
			);
		}
	...
	pause(): void {
		if (this.readStatus === "closed" || this.readStatus === "closing") {
			throw new StreamStateError(
				"Cannot pause a stream that is closing/closed",
			);
		}
	...
	resume(): void {
		if (this.readStatus === "closed" || this.readStatus === "closing") {
			throw new StreamStateError(
				"Cannot resume a stream that is closing/closed",
			);
		}
	...
	push(data: Uint8Array | Uint8ArrayList): void {
		if (this.readStatus === "closed" || this.readStatus === "closing") {
			throw new StreamStateError(
				`Cannot push data onto a stream that is ${this.readStatus}`,
			);
		}
	...
	onTransportClosed(err?: Error): void {
		...
		if (this.readStatus === "readable" && this.readBuffer.byteLength === 0) {
			this.log(
				"close readable end after transport closed and read buffer is empty",
			);
			this.readStatus = "closed";
		}
	...
	protected processSendQueue(): boolean {
		...
		this.log.trace(
			"processing send queue with %d queued bytes",
		...
		try {
			...
			while (this.writeBuffer.byteLength > 0) {
				const end = Math.min(
					this.maxMessageSize ?? this.writeBuffer.byteLength,
					this.writeBuffer.byteLength,
				);
	...
	protected dispatchReadBuffer(): void {
		try {
			if (this.listenerCount("message") === 0) {
				this.log.trace(
					"not dispatching pause buffer as there are no listeners for the message event",
				);
				return;
			}
			...
			if (this.readBuffer.byteLength === 0) {
				this.log.trace(
					"not dispatching pause buffer as there is no data to dispatch",
				);
				return;
			}
			...
			if (this.readStatus === "closing" || this.readStatus === "closed") {
				this.log(
					"dropping %d bytes because the readable end is %s",
			...
			if (
				this.readBuffer.byteLength === 0 &&
				this.remoteWriteStatus === "closed"
			) {
				this.log(
					"close readable end after dispatching read buffer and remote writable end is closed",
				);
				this.readStatus = "closed";
			}
