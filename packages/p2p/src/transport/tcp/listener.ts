...
import { getThinWaistAddresses } from "./utils";
	...
	implements Listener
{
	...
	constructor(private readonly context: Context) {
		...
		context.metrics?.registerMetricGroup(
			"libp2p_tcp_inbound_connections_total",
			{
				label: "address",
		...
		this.metrics = {
			status: context.metrics?.registerMetricGroup(
				"libp2p_tcp_listener_status_info",
				{
					label: "address",
			...
			errors: context.metrics?.registerMetricGroup(
				"libp2p_tcp_listener_errors_total",
				{
					label: "address",
			...
			events: context.metrics?.registerMetricGroup(
				"libp2p_tcp_listener_events_total",
				{
					label: "address",
				...
				socket.once("close", () => {
					...
					if (
						...
						this.resume().catch((err) => {
							this.log.error(
								"error attempting to listen server once connection count under limit - %e",
								err,
							);
