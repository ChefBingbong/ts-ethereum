...
export class MplexStreamMuxer extends AbstractStreamMuxer<MplexStream> {
	...
	constructor(maConn: AbstractMessageStream, init: MplexInit = {}) {
		...
		this.maxUnprocessedMessageQueueSize =
			init.maxUnprocessedMessageQueueSize ?? MAX_MSG_QUEUE_SIZE;
	...
	handleMessage(message: Message): void {
		...
		if (stream == null) {
			this.log(
				"missing stream %s for message type %s",
				id,
				MessageTypeNames[message.type],
			);
