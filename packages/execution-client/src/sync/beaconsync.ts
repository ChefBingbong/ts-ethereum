...
import { ReverseBlockFetcher } from './fetcher/index'
...
export class BeaconSynchronizer extends Synchronizer {
  ...
  get fetcher(): ReverseBlockFetcher | null {
    ...
    return this._fetcher as ReverseBlockFetcher | null
  }
  ...
  async open(): Promise<void> {
    ...
    const hardfork = this.config.hardforkManager.getHardforkByBlock(number)
      ...
      while (this.running) {
        try {
          ...
          this.config.options.logger?.error(
            `Beacon sync error: ${error.message}`,
          )
  ...
  async syncWithPeer(peer?: Peer): Promise<boolean> {
    ...
    if (first <= this.chain.blocks.height) {
      ...
      count = BigInt(this.config.options.skeletonSubchainMergeMinimum)
    } else {
  ...
  async processSkeletonBlocks(blocks: Block[]) {
    ...
    this.config.options.logger?.debug(
      `Imported skeleton blocks count=${blocks.length} first=${first} last=${last} hash=${hash} peers=${this.pool.getPeerCount()}`,
    )
  }
