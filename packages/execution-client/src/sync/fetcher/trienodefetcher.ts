...
import { getInitFetcherDoneFlags } from './types'
...
> {
  ...
  constructor(options: TrieNodeFetcherOptions) {
    ...
    this.keccakFunction = keccak256 as any
  ...
  async request(
    ...
    const rangeResult = await peer!.snap!.getTrieNodes({
      root: this.root,
      paths,
      bytes: BigInt(this.config.options.maxRangeBytes),
    })
    ...
    if (
      ...
      requestedNodeCount < rangeResult.nodes.length
    ) {
    ...
    try {
      ...
      for (let i = 0; i < rangeResult.nodes.length; i++) {
        ...
        const receivedHash = bytesToUnprefixedHex(
          this.keccakFunction(receivedNode) as Uint8Array,
        )
  ...
  async store(result: Uint8Array[]): Promise<void> {
    ...
    try {
      ...
      for (const nodeData of result[0]) {
        ...
        if (node instanceof BranchMPTNode) {
          ...
          for (const [i, embeddedNode] of children) {
            if (embeddedNode !== null) {
              const newStoragePath = nodePath.concat(
                bytesToUnprefixedHex(Uint8Array.from([i])),
              )
              const syncPath =
                storagePath === undefined
                  ? newStoragePath
                  : [accountPath, newStoragePath].join('/')
          ...
          const stringPath = bytesToUnprefixedHex(
            pathToHexKey(nodePath, node.key(), 'hex'),
          )
          const syncPath =
            storagePath === undefined
              ? stringPath
              : [accountPath, stringPath].join('/')
          const val = {
            nodeHash: node.value(),
            path: syncPath,
          }
          ...
          if (storagePath === undefined) {
            ...
            if (equalsBytes(storageRoot, KECCAK256_RLP) === false) {
              ...
              const syncPath = [
                bytesToUnprefixedHex(
                  pathToHexKey(accountPath, node.key(), 'hex'),
                ),
                storagePath,
              ].join('/')
        ...
        const { nodeParentHash, parentAccountHash } =
          this.pathToNodeRequestData.getElementByKey(
            pathString,
          ) as NodeRequestData
        if (storagePath !== undefined) {
          ...
          this.fetchedAccountNodes.set(nodeHash, {
            ...
            nodeData,
            path: pathString,
            pathToStorageNode: hasStorageComponent
        ...
        for (const [_, data] of this.fetchedAccountNodes) {
          ...
          if (node instanceof LeafMPTNode) {
            ...
            const storageTrie = new MerklePatriciaTrie({
              useKeyHashing: true,
              common: this.config.options.common,
            })
   ...
   */
  tasks(maxTasks = this.config.options.maxFetcherJobs): JobTask[] {
      ...
      if (this.pathToNodeRequestData.size() > 0) {
        ...
        while (tasks.length < maxTasks && pathStrings.length > 0) {
          ...
          for (const pathString of requestedPathStrings) {
            const nodeHash =
              this.pathToNodeRequestData.getElementByKey(pathString)?.nodeHash // TODO return node set too from sorted function and avoid lookups here
            if (nodeHash === undefined) throw Error('Path should exist')
            this.requestedNodeToPath.set(
              nodeHash as unknown as string,
              pathString,
            )
          }
          this.DEBUG && this.debug('At start of mergeAndFormatPaths')
          const paths = mergeAndFormatKeyPaths(
            requestedPathStrings,
   ...
   */
  peer(): Peer | undefined {
    return this.pool.getIdlePeer((peer) => 'snap' in peer)
  }
