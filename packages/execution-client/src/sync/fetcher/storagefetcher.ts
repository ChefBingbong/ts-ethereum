...
> {
  ...
  constructor(options: StorageFetcherOptions) {
    ...
    this.fetcherDoneFlags.storageFetcher.count = BigInt(
  ...
  async request(
    ...
    if (task.multi === false) {
      ...
      if (highestKnownHash && compareBytes(limit, highestKnownHash) < 0) {
        ...
        this.DEBUG &&
          this.debug(
            `skipping request with limit lower than highest known hash`,
          )
    ...
    if (
      ...
      task.storageRequests.length < rangeResult.slots.length
    ) {
      this.DEBUG &&
        this.debug(
    ...
    if (rangeResult.slots.length === 0) {
      ...
      if (rangeResult.proof.length > 0) {
        ...
        return Object.assign([], [Object.create(null) as any], {
          completed: true,
        })
      }
    ...
    for (let i = 0; i < rangeResult.slots.length; i++) {
      this.DEBUG &&
        this.debug(
          `number of slots in slot array ${i}: ${rangeResult.slots[i].length}`,
        )
    }
    this.DEBUG &&
    ...
    try {
      ...
      for (let i = 0; i < rangeResult.slots.length; i++) {
        ...
        for (let i = 0; i < accountSlots.length - 1; i++) {
          ...
          if (
            bytesToBigInt(accountSlots[i].hash) >
            bytesToBigInt(accountSlots[i + 1].hash)
          ) {
            throw Error(
        ...
        const proof =
          i === rangeResult.slots.length - 1 ? rangeResult.proof : undefined
  ...
  process(
    ...
    let fullResult: StorageData[][] | undefined
    ...
    if (result.completed === true) {
      ...
      if (updateHighestReceivedHash && highestReceivedhash !== undefined) {
        this.accountToHighestKnownHash.set(
          bytesToHex(request.accountHash),
          highestReceivedhash,
        )
      }
   ...
   */
  async store(
    result: StorageData[][] & { requests: StorageRequest[] } & {
      multi: boolean
    },
  ): Promise<void> {
    try {
      ...
      this.fetcherDoneFlags.storageFetcher.count =
        this.fetcherDoneFlags.storageFetcher.first +
        BigInt(this.storageRequests.length)
  ...
  tasks(
    ...
    const max = this.config.options.maxStorageRange
  ...
  processStoreError(
    ...
    const destroyFetcher =
      !error.message.includes(`InvalidRangeProof`) &&
      !error.message.includes(`InvalidStorageRange`)
    const banPeer = true
