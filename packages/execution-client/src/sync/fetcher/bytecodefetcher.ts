...
> {
  ...
  async request(
    ...
    this.DEBUG &&
      this.debug(
        `requested code hashes: ${Array.from(task.hashes).map((h) => bytesToHex(h))}`,
      )
    ...
    if (
      ...
      task.hashes.length < rangeResult.codes.length
    ) {
    ...
    if (missingCodeHashes.length > 0) {
      this.DEBUG &&
        this.debug(
          `${missingCodeHashes.length} missed requests adding them to request backlog`,
        )
  ...
  async store(result: Uint8Array[]): Promise<void> {
    ...
    this.fetcherDoneFlags.byteCodeFetcher.first += BigInt(
      codeHashToByteCode.size,
    )
  ...
  nextTasks(): void {
    this.DEBUG &&
      this.debug(
        `Entering nextTasks with hash request queue length of ${this.hashes.length}`,
      )
    ...
    try {
      if (this.in.length === 0 && this.hashes.length > 0) {
        ...
        this.DEBUG &&
          this.debug(
            `Fetcher pending with ${fullJob!.task.hashes.length} code hashes requested`,
          )
      }
    } catch (err) {
