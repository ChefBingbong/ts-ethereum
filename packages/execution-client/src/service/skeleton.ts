...
import { createBlockFromRLP } from '@ts-ethereum/block'
...
import { INVALID_FORKCHOICE_STATE } from '../rpc/error-code'
import type { SnapFetcherDoneFlags } from '../sync/fetcher/types'
import { short, timeDuration } from '../util/index'
 ...
 */
export const errReorgDenied = EthereumJSErrorWithoutCode(
  'non-forced head reorg denied',
)
...
export class Skeleton extends MetaDBManager {
  ...
  private filling =
    false /** Whether we are actively filling the canonical chain */
  ...
  private async fastForwardHead(lastchain: SkeletonSubchain, target: bigint) {
    ...
    for (
      ...
      if (
        newBlock === undefined ||
        !equalsBytes(newBlock.header.parentHash, headBlock.hash())
      ) {
  ...
  private async processNewHead(head: Block, force = false): Promise<boolean> {
    ...
    if (lastchain.tail > number) {
      ...
      if (
        mayBeDupBlock !== undefined &&
        equalsBytes(mayBeDupBlock.header.hash(), head.hash())
      ) {
        this.config.logger?.debug(
  ...
  updateSynchronizedState(latest?: BlockHeader | null) {
    ...
    const subchain0 = this.status.progress.subchains[0]
    if (
      (this.config.syncTargetHeight ?? BIGINT_0) === BIGINT_0 ||
      subchain0 === undefined
    ) {
      return
    }
    ...
    if (latest !== null && latest !== undefined) {
      ...
      if (height >= (this.config.syncTargetHeight ?? BIGINT_0)) {
        ...
        // update synchronized
        if (diff < this.config.options.syncedStateRemovalPeriod) {
  ...
  async forkchoiceUpdate(
    ...
    if (reorged && prevLinked && !this.status.linked) {
      ...
      await this.blockingTailBackfillWithCutoff(
        this.chain.config.options.engineParentLookupMaxDepth,
      ).catch((e) => {
        this.config.logger?.debug(
          `blockingTailBackfillWithCutoff exited with error=${e}`,
        )
      })
    }
    ...
    if (subchain0 === undefined) {
      throw Error(
        `subchain0 should have been set as a result of skeleton setHead`,
      )
    }
    ...
    if (safeBlockHash !== undefined) {
      if (equalsBytes(safeBlockHash, zeroBlockHash)) {
        ...
        equalsBytes(safeBlockHash, this.safeBlock?.hash() ?? zeroBlockHash)
      ) {
      ...
      if (safeBlock !== undefined) {
        ...
        if (
          !this.status.linked &&
          safeBlock.header.number === subchain0.tail - BIGINT_1
        ) {
    ...
    if (finalizedBlockHash !== undefined) {
      if (equalsBytes(finalizedBlockHash, zeroBlockHash)) {
        ...
        equalsBytes(
          finalizedBlockHash,
          this.finalizedBlock?.hash() ?? zeroBlockHash,
        )
      ) {
      ...
      if (finalizedBlock !== undefined) {
        if (
          ...
          (safeBlock !== undefined &&
            finalizedBlock.header.number > safeBlock.header.number)
        ) {
          throw {
            code: INVALID_PARAMS,
        ...
        if (
          !this.status.linked &&
          finalizedBlock.header.number === subchain0.tail - BIGINT_1
        ) {
    ...
    await this.runWithLock<void>(async () => {
      let shouldBeFinalizedNumber =
        this.finalizedBlock?.header.number ?? BIGINT_0
      if (
        finalizedBlock !== undefined &&
        finalizedBlock.header.number > shouldBeFinalizedNumber
      ) {
      ...
      if (
        ...
        this.safeBlock.header.number > shouldBeSafeNumber
      ) {
        shouldBeSafeNumber = this.safeBlock.header.number
      }
      if (
        safeBlock !== undefined &&
        safeBlock.header.number > shouldBeSafeNumber
      ) {
      ...
      if (this.status.linked || shouldBeSafeNumber >= subchain0.tail) {
        if (safeBlock === undefined) {
          ...
          const canonicalBlock = await this.getBlock(
            safeBlock.header.number,
            true,
          )
          if (
    ...
    await this.blockingFillWithCutoff(
      this.chain.config.options.engineParentLookupMaxDepth,
    )
        ...
        ((this.safeBlock !== undefined &&
          ...
          syncedBlock.header.number <=
            this.chain.blocks.height -
              this.config.options.snapTransitionSafeDepth)
      ) {
  ...
  private async trySubChainsMerge(): Promise<boolean> {
    ...
    while (
      ...
      if (tail >= this.status.progress.subchains[0].tail) {
        ...
        this.status.progress.subchains[1].head =
          this.status.progress.subchains[0].tail - BIGINT_1
        this.config.logger?.debug(
      ...
      const subChain1Head = await this.getBlock(
        this.status.progress.subchains[1].head,
        true,
      )
      // tail lookup also needs to be from skeleton because we set resetCanonicalHead true if merged
      const subChain1Tail = await this.getBlock(
        this.status.progress.subchains[1].tail,
        true,
      )
      if (
        ...
        !equalsBytes(
          subChain1Tail.header.parentHash,
          this.status.progress.subchains[1].next,
        )
      ) {
        ...
        this.status.progress.subchains.splice(
          1,
          this.status.progress.subchains.length - 1,
        )
      } else if (
        ...
        if (head - tail > this.config.options.skeletonSubchainMergeMinimum) {
          this.config.logger?.debug(
    ...
    return this.runWithLock<number>(async () => {
      ...
      this.config.logger?.debug(
        `Skeleton putBlocks start=${blocks[0]?.header.number} hash=${short(
          blocks[0]?.hash(),
        )} fork=${blocks[0].header.hardfork} end=${
      ...
      for (const block of blocks) {
        ...
        if (equalsBytes(this.status.progress.subchains[0].next, block.hash())) {
          ...
          const tailBlock = await this.getBlock(
            this.status.progress.subchains[0].tail,
          )
          this.config.logger?.warn(
            ...
            )} tailFork=${tailBlock?.header.hardfork}, block number=${number} tailparent=${short(
              tailBlock?.header.parentHash ?? zeroBlockHash,
            )} hash=${short(block.hash())} fork=${block.header.hardfork}`,
          )
  ...
  private async backStep(fromBlock: bigint): Promise<bigint | null> {
    try {
      if (this.config.options.skeletonFillCanonicalBackStep <= 0) return null
      ...
      do {
        newTail =
          newTail + BigInt(this.config.options.skeletonFillCanonicalBackStep)
      ...
      if (tailBlock !== undefined && newTail) {
        this.config.logger?.info(
          `Backstepped skeleton tail=${newTail} head=${head}`,
        )
  ...
  async blockingFillWithCutoff(cutoffLen: number): Promise<void> {
    ...
    if (this.status.linked && subchain0 !== undefined) {
      ...
      const fillPromise = this.fillCanonicalChain(isChainHeadNearEnough).catch(
        (_e) => {},
      )
  ...
  async tryTailBackfill(): Promise<void> {
    let blocks: Block[]
    do {
      blocks = await this.getUnfinalizedParentsForBackfill(
        this.chain.config.options.maxPerRequest,
      )
      if (blocks.length > 0) {
      ...
      if (!this.status.linked && blocks.length === maxItems) {
        void this.tryTailBackfill().catch((e) => {
          this.chain.config.logger?.debug(
            `tryTailBackfill exited with error ${e}`,
          )
        })
      }
    }
  ...
  async fillCanonicalChain(skipUpdateEmit = false) {
    ...
    while (!this.status.canonicalHeadReset && canonicalHead < subchain.head) {
      ...
      if (this.chain.blocks.height <= block.header.number) {
        try {
          numBlocksInserted = await this.chain.putBlocks(
            [block],
            true,
            skipUpdateEmit,
          )
          ...
          this.config.logger?.error(
            `fillCanonicalChain putBlock error=${validationError}`,
          )
        ...
        if (numBlocksInserted !== 1) {
          this.config.logger?.error(
            `Failed to put block number=${number} fork=${block.header.hardfork} hash=${short(
              block.hash(),
          ...
          try {
            ...
            this.config.logger?.info(
              ...
              )} hf=${parent?.header.hardfork}`,
            )
          } catch {
            this.config.logger?.error(
              `Failed to fetch parent of number=${number}`,
            )
          }
          ...
          try {
            ...
            this.config.logger?.info(
              ...
              )} hf=${parentWithHash?.header.hardfork}  `,
            )
          } catch {
            this.config.logger?.error(
      ...
      if (fillLogIndex >= this.config.options.numBlocksPerIteration) {
        this.config.logger?.debug(
  ...
  private async putBlock(
    ...
    const rlp = this.serialize({
      hardfork: block.header.hardfork,
      blockRLP: block.serialize(),
    })
  ...
  skeletonBlockRlpToBlock(skeletonBlockRlp: Uint8Array): Block {
    const { blockRLP } = this.deserialize(skeletonBlockRlp)

    const block = createBlockFromRLP(blockRLP, {
      hardforkManager: this.config.options.common,
    })
    return block
  }
  ...
  async getBlock(
    ...
    try {
      const skeletonBlockRlp = await this.get(
        DBKey.SkeletonBlock,
        bigIntToBytes(number),
      )
      if (skeletonBlockRlp === null) {
        throw Error(
          `SkeletonBlock rlp lookup failed for ${number} onlyCanonical=${onlyCanonical}`,
        )
      }
  ...
  async getBlockByHash(
    ...
    let block =
      onlyCanonical === false ? await this.getUnfinalizedBlock(hash) : undefined
    if (
      block === undefined &&
      (onlyCanonical === false || this.status.linked)
    ) {
    ...
    if (onlyCanonical === false) {
      ...
      if (this.status.linked && block !== undefined) {
        const canBlock = await this.chain
          .getBlock(block.header.number)
          .catch((_e) => undefined)
        if (
          canBlock !== undefined &&
          equalsBytes(canBlock.hash(), block.hash())
        ) {
          // block is canonical
          return block
        }
  ...
  async getUnfinalizedBlock(hash: Uint8Array): Promise<Block | undefined> {
    try {
      const skeletonBlockRlp = await this.get(
        DBKey.SkeletonUnfinalizedBlockByHash,
        hash,
      )
      if (skeletonBlockRlp === null) {
        throw Error(
          `SkeletonUnfinalizedBlockByHash rlp lookup failed for hash=${short(hash)}`,
        )
      }
      return this.skeletonBlockRlpToBlock(skeletonBlockRlp)
    } catch {
  ...
  logSyncStatus(
    ...
    const isSynced =
      this.status.linked &&
      (this.chain.blocks.latest?.header.number ?? BIGINT_0) ===
        (subchain0?.head ?? BIGINT_0)
    ...
    if (status !== 'EXECUTING') {
      ...
      if (
        this.lastexecutedAt === 0 ||
        this.lastexecuted !== vmHead?.header.number
      ) {
    ...
    switch (status) {
      case 'EXECUTING':
        scenario =
          Date.now() - this.lastexecutedAt > STALE_WINDOW
            ? 'execution stalled?'
            : ''
        ...
        if (vmexecution?.started === true) {
          ...
          if (snapsync.done) {
            scenario = `snapsync-to-vm-transition=${
              (snapsync.snapTargetHeight ?? BIGINT_0) +
              this.config.options.snapTransitionSafeDepth
            }`
          } else {
      ...
      case 'SYNCING':
        if (this.filling) {
          scenario =
            Date.now() - this.lastfilledAt > STALE_WINDOW
              ? 'filling stalled?'
              : 'filling'
    ...
    if (forceShowInfo || status !== lastStatus) {
      ...
      if (isValid) {
        this.config.logger?.info(
          `${logPrefix} ${status}${extraStatus} ${vmlogInfo} peers=${peers}`,
        )
      } else {
        // else break into two
        this.config.logger?.info(
  ...
  private statusToRLP(): Uint8Array {
    const subchains: SkeletonSubchainRLP[] = this.status.progress.subchains.map(
      (subchain) => [
