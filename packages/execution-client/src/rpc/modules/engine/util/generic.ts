...
export const recursivelyFindParents = async (
  ...
  if (
    equalsBytes(parentHash, vmHeadHash) ||
    equalsBytes(parentHash, new Uint8Array(32))
  ) {
    return []
  }
  ...
  while (
    !equalsBytes(parentBlocks[parentBlocks.length - 1].hash(), vmHeadHash)
  ) {
    ...
    if (parentBlocks.length > maxDepth) {
      throw Error(
        `recursivelyFindParents lookups deeper than maxDepth=${maxDepth}`,
      )
    }
  }
...
export const validExecutedChainBlock = async (
  ...
  try {
    const block =
      blockOrHash instanceof Uint8Array
        ? await chain.getBlock(blockOrHash)
        : blockOrHash
    ...
    if (vmHead.header.number >= block.header.number) {
      ...
      const canonicalHash = await chain.blockchain.safeNumberToHash(
        block.header.number,
      )
      if (
        canonicalHash instanceof Uint8Array &&
        equalsBytes(block.hash(), canonicalHash)
      ) {
        return block
      }
    }
  ...
  try {
    ...
    for (
      ...
      const isBlockExecuted =
        (executedBlocks.get(unPrefixedHashStr) ??
          (await validExecutedChainBlock(hash, chain))) !== null
      if (isBlockExecuted) {
...
/**
 * Validates that the method version is appropriate for the given timestamp.
 */
export function validateHardforkRange(
  hardforkManager: HardforkManager,
  methodVersion: number,
  checkNotBeforeHf: string | null,
  checkNotAfterHf: string | null,
  timestamp: bigint,
) {
  if (checkNotBeforeHf !== null) {
    const hfTimeStamp = hardforkManager.hardforkTimestamp(checkNotBeforeHf)
    if (hfTimeStamp !== undefined && timestamp < hfTimeStamp) {
      throw {
  ...
  if (checkNotAfterHf !== null) {
    // Get the next hardfork's timestamp to validate we're not past the valid range
    const hardforks = hardforkManager.hardforks()
    const currentIdx = hardforks.findIndex((h) => h.name === checkNotAfterHf)
    if (currentIdx !== -1 && currentIdx < hardforks.length - 1) {
      const nextHf = hardforks[currentIdx + 1]
      const nextHFTimestamp =
        nextHf.timestamp !== undefined ? BigInt(nextHf.timestamp) : undefined
      if (nextHFTimestamp !== undefined && timestamp >= nextHFTimestamp) {
        throw {
          code: UNSUPPORTED_FORK,
