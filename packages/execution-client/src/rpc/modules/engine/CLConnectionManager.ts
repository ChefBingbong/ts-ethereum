...
import { short, timeDiff } from '../../../util/index'
import type {
  ExecutionPayloadV1,
  ExecutionPayloadV2,
  ExecutionPayloadV3,
...
export class CLConnectionManager {
  ...
  constructor(opts: CLConnectionManagerOpts) {
    ...
    if (
      this.config.hardforkManager.hardforkGte(
        this.config.hardforkManager.getLatestHardfork(),
  ...
  private _getPayloadLogMsg(payload: NewPayload) {
    ...
    if (
      'withdrawals' in payload.payload &&
      payload.payload.withdrawals !== null
    ) {
      msg += ` withdrawals=${(payload.payload as ExecutionPayloadV2).withdrawals.length}`
    }
    if (
      'excessBlobGas' in payload.payload &&
      payload.payload.excessBlobGas !== null
    ) {
  ...
  private connectionCheck() {
    ...
    if (
      ...
      this.config.hardforkManager.getLatestHardfork() === Hardfork.Paris
    ) {
