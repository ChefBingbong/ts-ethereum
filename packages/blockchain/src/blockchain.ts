...
export class Blockchain implements BlockchainInterface {
  ...
  private async findCommonAncestor(newHeader: BlockHeader) {
    ...
    this.DEBUG &&
      this._debug(
        `found common ancestor with hash=${bytesToHex(header.hash())}`,
      )
    this.DEBUG &&
      this._debug(`total ancestor headers num=${ancestorHeaders.size}`)
    return {
  ...
  private async _deleteCanonicalChainReferences(
    ...
    try {
      ...
      this.DEBUG &&
        ...
        this._debug(
          `deleted ${this._deletedBlocks.length} stale canonical blocks in total`,
        )
    } catch (e) {
