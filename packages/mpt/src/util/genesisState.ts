...
export async function genesisMPTStateRoot(genesisState: GenesisState) {
  ...
  for (const [key, value] of Object.entries(genesisState)) {
    const address = isHexString(key)
      ? hexToBytes(key)
      : unprefixedHexToBytes(key)
    ...
    if (typeof value === 'string') {
      ...
      if (storage !== undefined) {
        ...
        for (const [k, val] of storage) {
          const storageKey = isHexString(k)
            ? hexToBytes(k)
            : unprefixedHexToBytes(k)
          const storageVal = RLP.encode(
            unpadBytes(
              isHexString(val) ? hexToBytes(val) : unprefixedHexToBytes(val),
            ),
          )
