...
import { isHexString, padToEven, stripHexPrefix } from './internal'
 ...
 */
const stripZeros = <
  T extends Uint8Array | number[] | string = Uint8Array | number[] | string,
>(
  a: T,
): T => {
  let first = a[0]
 ...
 */
export function matchingBytesLength(
  bytes1: Uint8Array,
  bytes2: Uint8Array,
): number {
  let count = 0
