...
export function getBinaryTreeStem(
	...
	const treeStem = hashFunction(concatBytes(address32, treeIndexBytes)).slice(
		0,
		31,
	);
...
export type BinaryTreeLeafType =
	(typeof BinaryTreeLeafType)[keyof typeof BinaryTreeLeafType];
...
export const BINARY_TREE_BASIC_DATA_LEAF_KEY = intToBytes(
	BinaryTreeLeafType.BasicData,
);
export const BINARY_TREE_CODE_HASH_LEAF_KEY = intToBytes(
...
export const BINARY_TREE_MAIN_STORAGE_OFFSET =
	BigInt(256) ** BigInt(BINARY_TREE_CODE_CHUNK_SIZE);
 ...
 */
export const getBinaryTreeKey = (
	stem: Uint8Array,
	leaf: BinaryTreeLeafType | Uint8Array,
) => {
	switch (leaf) {
...
export function getBinaryTreeIndicesForStorageSlot(storageKey: bigint): {
	...
	if (
		storageKey <
		BINARY_TREE_CODE_OFFSET - BINARY_TREE_HEADER_STORAGE_OFFSET
	) {
...
export function getBinaryTreeIndicesForCodeChunk(chunkId: number) {
	const treeIndex = Math.floor(
		(BINARY_TREE_CODE_OFFSET + chunkId) / BINARY_TREE_NODE_WIDTH,
	);
...
export const getBinaryTreeKeyForCodeChunk = (
	...
	return concatBytes(
		getBinaryTreeStem(hashFunction, address, treeIndex),
		intToBytes(subIndex),
	);
};
...
export const getBinaryTreeKeyForStorageSlot = (
	...
	const { treeIndex, subIndex } =
		getBinaryTreeIndicesForStorageSlot(storageKey);
...
export function decodeBinaryTreeLeafBasicData(
	...
	const versionBytes = encodedBasicData.slice(
		0,
...
export function generateBinaryTreeCodeStems(
	...
	const numStems =
		numChunks > BINARY_TREE_CODE_OFFSET
			? Math.ceil(numChunks / BINARY_TREE_NODE_WIDTH) + 1
			: 1;
	const chunkStems = new Array<Uint8Array>(numStems);
	// Compute the stem for the initial set of code chunks
	chunkStems[0] = getBinaryTreeKeyForCodeChunk(address, 0, hashFunction).slice(
		0,
		31,
	);
